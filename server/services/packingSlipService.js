...
// Logo paths
//   LOGO_PATH         — top-of-slip header logo (centered above title bar)
//   FOOTER_LOGO_PATH  — bottom-right footer logo, sits at the corner of the slip
...
class PackingSlipService {
   ...
   * @param {object} teamOptions - Optional rendering hints:
   *   - team {string}                   Active team filter (highlights items for this team)
   *   - teamItems {string[]}            Item IDs belonging to the active team
   *   - athlete {{firstName,lastName,customerOrderNum}}  Bulk-style athlete header
   *   - filenameOverride {string}       Custom filename (without dir). When set, replaces
   *                                     the default "<order.num>_packing_slip[_<team>].jpg".
   *                                     Required for bulk per-dancer slips so each dancer's
   *                                     slip gets a unique filename instead of overwriting.
  ...
  async generateSlip(order, orderDir, teamOptions = {}) {
    ...
    const isBulk = !!order.isBulkOrder || (order.groups || []).length > 1;
    ...
    // Bulk-mode athlete option: when present, use a Bulk-specific header that
    // puts the athlete name big at the top instead of the standard logo+title bar.
    // teamOptions.athlete = { firstName, lastName, customerOrderNum }
    const athlete = teamOptions.athlete || null;
    const useBulkHeader = !!athlete;
    ...
    // Reserve space depending on header style:
    //   Standard:  logo (~170px) + title bar (80px) + order info (~460px) + ship-to (~360px) + items header (35px) + dividers (~40px) = ~1145px
    //   Bulk:      athlete-name header zone (~290px) + order info (~460px) + ship-to (~360px) + items header (35px) + dividers (~40px) = ~1185px
    // Both leave roughly the same room for items.
    const headerSpace = useBulkHeader
      ? (290 + 460 + 360 + 35 + 40)
      : (170 + 80 + 460 + 360 + 35 + 40);
    ...
    if (useBulkHeader) {
      // Bulk header: NO top logo and NO "PACKING SLIP" title bar. The athlete name
      // dominates the top of the slip so studio staff can flip through a stack and
      // identify each dancer's order at a glance like an index card.
      const lastName = (athlete.lastName || '').trim();
      const firstName = (athlete.firstName || '').trim();
      // "LASTNAME, FirstName" format — last name in caps for emphasis & matches sort order.
      const nameLine = lastName
        ? (firstName ? `${lastName.toUpperCase()}, ${firstName}` : lastName.toUpperCase())
        : (firstName || 'UNKNOWN');

      // Auto-fit font size with a hard 0.25" (75px at 300 DPI) padding on each
      // side so the name never gets clipped at the printer's bleed edge. Math:
      //   usable width = SLIP_WIDTH (1500) - 2 * 75 = 1350px
      // The char-width ratio for Arial Black is bumped from 0.55 → 0.62 because
      // the previous estimate undershot for long names like "FRANASCHOUK, Rylah",
      // letting them run right up to (and sometimes past) the safe edge.
      const NAME_SIDE_PADDING = 75; // 0.25" at 300 DPI
      const maxNameWidth = SLIP_WIDTH - (NAME_SIDE_PADDING * 2);
      const charWidthRatio = 0.62; // Arial Black is wide; conservative estimate
      const idealSize = Math.floor(maxNameWidth / (Math.max(nameLine.length, 1) * charWidthRatio));
      let nameFontSize = Math.max(40, Math.min(180, idealSize));

      // Belt-and-suspenders: hard-clamp via SVG textLength so the rendered text
      // physically cannot exceed the safe area, regardless of font metric drift.
      // SVG renderers honor textLength by squeezing/scaling the glyphs to fit.
      const estimatedRenderedWidth = nameLine.length * charWidthRatio * nameFontSize;
      const useTextLengthClamp = estimatedRenderedWidth > maxNameWidth;

      // Push name down so it's visually centered vertically in the top header zone
      const headerZoneHeight = 280;
      const nameBaselineY = MARGIN + Math.round(headerZoneHeight / 2) + Math.round(nameFontSize / 3);

      const textLengthAttr = useTextLengthClamp
        ? ` textLength="${maxNameWidth}" lengthAdjust="spacingAndGlyphs"`
        : '';
      svgParts.push(
        `<text x="${SLIP_WIDTH / 2}" y="${nameBaselineY}" ` +
        `font-family="Arial Black, Arial, sans-serif" font-size="${nameFontSize}" font-weight="900" ` +
        `fill="#1a1a1a" text-anchor="middle"${textLengthAttr}>${this._esc(nameLine)}</text>`
      );

      y = MARGIN + headerZoneHeight + 10;
      // Subtle divider beneath athlete name
      svgParts.push(`<line x1="${MARGIN}" y1="${y}" x2="${SLIP_WIDTH - MARGIN}" y2="${y}" stroke="#cccccc" stroke-width="2"/>`);
      y += 25;
    } else {
      // Standard header: top logo + "PACKING SLIP" title bar
    ...
    // Doubled from previous (label 22→44, value 24→48) for better readability at packing-table distance
    const labelStyle = 'font-family="Arial, sans-serif" font-size="44" fill="#888888"';
    const valueStyle = 'font-family="Arial, sans-serif" font-size="48" font-weight="bold" fill="#222222"';

    // Each column gets ~half the slip width minus margins. Constrain text so values
    // never overflow into the adjacent column. SVG textLength/lengthAdjust support
    // is inconsistent across renderers, so we just truncate with ellipsis.
    // At 48pt font, the column fits about 22 chars before crowding the next column.
    const MAX_VAL_CHARS = 22;
    const fitValue = (x, yPos, text) => {
      let t = text == null ? '' : String(text);
      if (t.length > MAX_VAL_CHARS) t = t.slice(0, MAX_VAL_CHARS - 1).trimEnd() + '…';
      return `<text x="${x}" y="${yPos}" ${valueStyle}>${this._esc(t)}</text>`;
    };

    // Order number to display: prefer the athlete's customer-order-num when in
    // bulk header mode (so each dancer's slip shows their own order number,
    // not the parent bulk order's). Falls back to order.num for non-bulk slips
    // and for merged dancers where multiple customer order numbers collapsed
    // and the caller passed the parent bulk number through.
    const displayOrderNum = (useBulkHeader && athlete && athlete.customerOrderNum)
      ? athlete.customerOrderNum
      : (order.num || '');

    // Row 1: Order # and Date
    svgParts.push(`<text x="${leftCol}" y="${y + 44}" ${labelStyle}>Order #</text>`);
    svgParts.push(fitValue(leftCol, y + 104, displayOrderNum));
    const dateStr = order.placedAt ? new Date(order.placedAt).toLocaleDateString() : '';
    svgParts.push(`<text x="${rightCol}" y="${y + 44}" ${labelStyle}>Order Date</text>`);
    svgParts.push(fitValue(rightCol, y + 104, dateStr));
    y += 150;

    // Row 2: Gallery and Shipping Option
    svgParts.push(`<text x="${leftCol}" y="${y + 44}" ${labelStyle}>Gallery</text>`);
    svgParts.push(fitValue(leftCol, y + 104, order.gallery || 'N/A'));
    svgParts.push(`<text x="${rightCol}" y="${y + 44}" ${labelStyle}>Shipping</text>`);
    svgParts.push(fitValue(rightCol, y + 104, shippingOption));
    y += 150;

    // Row 3: Order Type and Studio
    svgParts.push(`<text x="${leftCol}" y="${y + 44}" ${labelStyle}>Order Type</text>`);
    svgParts.push(fitValue(leftCol, y + 104, isBulk ? 'Bulk Order' : 'Dropship'));
    svgParts.push(`<text x="${rightCol}" y="${y + 44}" ${labelStyle}>Studio</text>`);
    svgParts.push(fitValue(rightCol, y + 104, studioName));
    y += 160;
    ...
    // ─── Ship To ─────────────────────────────────────────
    svgParts.push(`<text x="${leftCol}" y="${y + 44}" ${labelStyle}>SHIP TO</text>`);
    y += 70;
    svgParts.push(`<text x="${leftCol}" y="${y + 56}" font-family="Arial, sans-serif" font-size="56" font-weight="bold" fill="#222222">${this._esc(dest.recipient || '')}</text>`);
    y += 72;
    if (dest.address1) {
      svgParts.push(`<text x="${leftCol}" y="${y + 48}" font-family="Arial, sans-serif" font-size="44" fill="#333333">${this._esc(dest.address1)}</text>`);
      y += 56;
    }
    if (dest.address2) {
      svgParts.push(`<text x="${leftCol}" y="${y + 48}" font-family="Arial, sans-serif" font-size="44" fill="#333333">${this._esc(dest.address2)}</text>`);
      y += 56;
    }
    const cityLine = [dest.city, dest.state].filter(Boolean).join(', ') + (dest.zipCode ? ` ${dest.zipCode}` : '');
    if (cityLine.trim()) {
      svgParts.push(`<text x="${leftCol}" y="${y + 48}" font-family="Arial, sans-serif" font-size="44" fill="#333333">${this._esc(cityLine)}</text>`);
      y += 56;
    }
    if (dest.phone || dest.phoneNumber) {
      svgParts.push(`<text x="${leftCol}" y="${y + 44}" font-family="Arial, sans-serif" font-size="40" fill="#666666">${this._esc(dest.phone || dest.phoneNumber)}</text>`);
      y += 52;
    }
    y += 20;

    // ─── Bulk order group fields ─────────────────────────
    // Skip this section when the bulk-style athlete header is in use — the big
    // athlete name at the top of the slip already identifies the dancer, and
    // showing all groups' fields here would dump every dancer's info onto
    // every per-dancer slip.
    if (isBulk && !useBulkHeader && order.groups?.length > 0) {
      svgParts.push(`<line x1="${MARGIN}" y1="${y}" x2="${SLIP_WIDTH - MARGIN}" y2="${y}" stroke="#dddddd" stroke-width="2"/>`);
      y += 15;
    ...
    svgParts.push(`<text x="${contactX}" y="${contactY + 8}" font-family="Arial, sans-serif" font-size="16" fill="#aaaaaa">Order: ${this._esc(displayOrderNum)}</text>`);
    ...
    // Add QR code
    try {
      const qrDataUrl = await QRCode.toDataURL(displayOrderNum || 'NO_ORDER', {
        width: QR_SIZE,
        margin: 1,
    ...
    // Add top header logo if available
    if (logoBuffer) {
    ...
    // Add bottom-right footer logo if available
    if (await fs.pathExists(FOOTER_LOGO_PATH)) {
      try {
        const footerLogoBuffer = await fs.readFile(FOOTER_LOGO_PATH);
        const flMeta = await sharp(footerLogoBuffer).metadata();
        // Footer logo: max 400px wide × 140px tall, fit-inside scaling
        const maxFw = 400;
        const maxFh = 140;
        const fScale = Math.min(maxFw / flMeta.width, maxFh / flMeta.height, 1);
        const flw = Math.round(flMeta.width * fScale);
        const flh = Math.round(flMeta.height * fScale);
        const resized = await sharp(footerLogoBuffer).resize(flw, flh).png().toBuffer();
        composites.push({
          input: resized,
        ...
        console.error('[PackingSlip] Footer logo load error:', flErr.message);
      }
    }
    ...
    // Filename precedence:
    //   1. Explicit teamOptions.filenameOverride (e.g., bulk per-dancer slips like
    //      "001_Augst_Adelyn_packing_slip.jpg") — required when generating multiple
    //      slips for the same order so they don't clobber each other.
    //   2. Default: "<order.num>_packing_slip[_<team>].jpg"
    const filenameOverride = teamOptions.filenameOverride;
    let filename;
    if (filenameOverride && typeof filenameOverride === 'string') {
      // Sanitize the override to prevent path traversal / invalid Windows chars
      const cleaned = filenameOverride.replace(/[<>:"/\\|?*]/g, '_').replace(/\s+/g, '_');
      // Ensure .jpg extension
      filename = cleaned.toLowerCase().endsWith('.jpg') ? cleaned : `${cleaned}.jpg`;
    } else {
      const safeTeamName = activeTeam ? '_' + activeTeam.replace(/[<>:"/\\|?*]/g, '_').replace(/\s+/g, '_') : '';
      filename = `${order.num}_packing_slip${safeTeamName}.jpg`;
    }
